...
export function extractFromAmazon(documentRoot: Document, url: string): ExtractedProduct {
  ...
  const landing = documentRoot.querySelector<HTMLImageElement>('#landingImage, img#imgBlkFront, img[data-old-hires]');
  const dyn = landing?.getAttribute('data-a-dynamic-image');
  if(dyn){
    try { Object.keys(JSON.parse(dyn)).forEach(u => imgs.add(u)); } catch{}
  }

  documentRoot.querySelectorAll<HTMLImageElement>('#altImages img, .image.item img, #ebooksImageBlockContainer img')
    .forEach(img => {
      const u = img.getAttribute('srcset')?.split(',')?.pop()?.trim().split(' ')?.[0]
        || img.getAttribute('data-old-hires')
        || img.src;
      if(u) imgs.add(u.replace(/\._SX\d+_.*/, ''));
    });
  ...
  if(ld?.image){
    (Array.isArray(ld.image) ? ld.image : [ld.image]).forEach((u: string) => imgs.add(u));
  }
