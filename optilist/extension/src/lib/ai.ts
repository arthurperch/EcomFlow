import { ExtractedProduct, OptimizedListing } from "../types";
...
export async function optimizeForEbay(p: ExtractedProduct): Promise<OptimizedListing> {
  ...
  if(!json?.ebayTitle || !json?.ebayHtmlDescription) throw new Error("Bad LLM output");
